...
const FORMATY = [
  ...
  /*
    WLASNY FORMAT KANONICZNY z godzina.

    Bez tego wpisu aplikacja nie potrafila odczytac tego, co sama zapisuje:
    przy odtwarzaniu z wlasnej kopii zapasowej 456 z 526 zadan odpadalo z bledem
    'nierozpoznany format daty "2026-08-16T20:25"'. Kopia, ktorej nie da sie
    wczytac z powrotem, nie jest kopia zapasowa.

    Dopuszczamy tez spacje zamiast T oraz sekundy - dokladnie ten sam zestaw
    wariantow co znormalizujZnacznikCzasu, zeby oba poziomy (API i import)
    przyjmowaly to samo.
  */
  {
    nazwa: 'YYYY-MM-DDTHH:MM',
    wzorzec: /^(\d{4})-(\d{1,2})-(\d{1,2})[T ](\d{1,2}):(\d{2})(?::\d{2})?$/,
    zloz: (m) =>
      zlozZGodzina(Number(m[1]), Number(m[2]), Number(m[3]), Number(m[4]), Number(m[5])),
  },
  {
    nazwa: 'Month D, YYYY',
