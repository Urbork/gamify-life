/*
  ...
  dziennik i profil quest-log korzystaja z tego samego kodu, podajac wlasny
 ...
 * @property {(surowy: object) => boolean} [filtrWierszy]
 *           przepuszcza tylko wybrane wiersze pliku; wiersze odrzucone NIE trafiaja
 *           ani do gotowych, ani do odrzuconych - po prostu nie dotycza tego przebiegu.
 *           Pozwala przepuscic ten sam plik dwa razy z roznymi mapowaniami
 *           (np. Type=Project -> projekty, Type=Task -> zadania).
 ...
 * @property {(rekord: object, surowy: object) => void} [poWierszu]
 *           hook uruchamiany PO zmapowaniu wszystkich pol, przed walidacja.
 *           Sluzy korektom zaleznym od kilku pol naraz - np. "pole B nadpisuje
 *           pole A, ale tylko gdy B jest niepuste". Samo mapowanie tego nie potrafi:
 *           dwa naglowki wskazujace na to samo pole przetwarzaja sie po kolei,
 *           a pusta wartosc ustawia null i kasuje to, co wpisal poprzedni naglowek.
...
function przygotujImport(tresc, konfiguracja) {
  const {
    ...
    kolumnyWymagane = [],
    kolumnyIgnorowane = [],
    // Domyslnie przepuszczamy wszystko - profile, ktore tego nie podaja,
    // zachowuja sie dokladnie tak jak przed dodaniem tej opcji.
    filtrWierszy = () => true,
    polaDatowe = [],
  ...
  for (const { dane, linia } of rekordy) {
    // Wiersz spoza tego przebiegu pomijamy w calosci - nie jest ani gotowy, ani odrzucony.
    if (!filtrWierszy(dane)) continue;
    ...
    // Korekty zalezne od kilku pol naraz - przed walidacja, bo moga wplynac na jej wynik.
    if (powod === null && poWierszu) poWierszu(docelowy, dane);
