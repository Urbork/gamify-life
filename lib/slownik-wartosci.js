...
function utworzRouterSlownika(db, opcje) {
  ...
  function idZParametru(req) {
    ...
    if (!Number.isInteger(id) || id <= 0) throw blad(400, `Niepoprawne id ${dopelniacz}.`);
    return id;
  }
  ...
  function nazwaZZadania(req) {
    ...
    if (typeof nazwa !== 'string' || nazwa.trim() === '') {
      throw blad(400, `Nazwa ${dopelniacz} nie może być pusta.`);
    }
    ...
    */
    if (przycieta.includes(',')) {
      throw blad(
        400,
        `Nazwa ${dopelniacz} nie może zawierać przecinka — przecinek rozdziela nazwy w dzienniku.`
      );
    }
  ...
  */
  ...
  router.post('/', (req, res) => {
    ...
    if (istniejacy) {
      throw blad(409, `${mianownik} "${istniejacy.nazwa}" już istnieje na liście.`);
    }
  ...
  router.patch('/:id', (req, res) => {
    ...
    if (!pozycja) throw blad(404, `Nie ma ${dopelniacz} o id ${id}.`);
    ...
    if (kolizja && kolizja.id !== id) {
      throw blad(409, `${mianownik} "${kolizja.nazwa}" już istnieje na liście.`);
    }
